...
def print_text(message, x, y, font_type=pygame.font.SysFont('Times new roman', 30), font_size=30,
               font_color=Common.WHITE):
...
def initialize_parameters():
    global start_game, running, end_game, hello_text, user_name, prompt_text, end_game_text, top5_text, prompt, \
        player, count_asteroid, i
    start_game = False
    running = False
    end_game = False
    hello_text = 'Введите Ваше имя:'
    ...
    Common.score = 0
    Common.lives = 5
    Common.asteroids.clear()
    Common.bullets.clear()
    Common.all_sprites.empty()
    # Создаем игрока и астероиды
    player = Player.PlayerObject()
    count_asteroid = random.randint(3, 6)
    for i in range(count_asteroid):
...
# Цикл начала игры
def start_game_loop():
    global start_game, prompt, running, user_name, prompt_text, restart
    ...
    while not start_game:
        ...
        if restart:
            ...
            if not prompt:
                print_text(hello_text, Common.center_width - Common.half_font_size * (len(hello_text) / 2),
                           Common.center_height - 100)
                print_text(user_name, Common.center_width - Common.half_font_size * (len(user_name) / 2),
                           Common.center_height - 60)
                print_text(prompt_text, Common.center_width - Common.half_font_size * (len(prompt_text) / 2),
                           Common.center_height - 15)
            else:
                # Если показывается подсказка
                prompt_text = 'Правила игры:'
                print_text(prompt_text, Common.center_width - Common.half_font_size * (len(prompt_text) / 2),
                           Common.center_height - Common.font_size * 3)
                prompt_text = 'Управление - стрелками. Для выстрела нажмите пробел'
                print_text(prompt_text, Common.center_width - Common.half_font_size * (len(prompt_text) / 2),
                           Common.center_height - Common.font_size * 2)
    ...
    while running:
        ...
        if restart:
            ...
            for asteroid in Common.asteroids:
                for bul in Common.bullets:
                    if asteroid.rect.left <= bul[0] <= asteroid.rect.right and asteroid.rect.top - 10 <= bul[
            ...
            for health in range(Common.lives):
                screen.blit(Common.health_img, (
                    Common.center_width - Common.health_img.get_rect().width * health + Common.health_img.get_rect().width,
                    0))
...
# Цикл окончания игры
def end_game_loop():
    global end_game, end_game_text, restart
    end_game = True
    ...
    while end_game:
        ...
        if restart:
            ...
            print_text(end_game_text, Common.center_width - Common.half_font_size * (len(end_game_text) / 2),
                       Common.indent_user_score)
            print_text(top5_text, Common.center_width - Common.half_font_size * (len(top5_text) / 2),
                       Common.indent_user_score + Common.font_size)
            # Выводится топ-5 игроков
            user_scores = database.get_users_score()
            i = 1
            for user_score in user_scores:
                print_text(user_score[0],
                           Common.center_width - Common.half_font_size * (len(user_score[0]) / 2) * 2,
                           (Common.indent_user_score + Common.font_size * 2) + i * Common.font_size)
                print_text(str(user_score[1]),
                           Common.center_width - Common.half_font_size * (
                                   len(str(user_score[1])) / 2) * 2 + Common.font_size * 2,
                           (Common.indent_user_score + Common.font_size * 2) + i * Common.font_size)
                i += 1
            restart_game_text = 'Для перезапуска игры нажмите R...'
            print_text(restart_game_text, Common.center_width - Common.half_font_size * (len(restart_game_text) / 2),
                       (Common.indent_user_score + Common.font_size * 2) + i * Common.font_size, font_color=Common.RED)
...
while restart:
    start_game_loop()
    # Сделано так, потому что игрок может нажать выход из программы еще во время ввода имени
    # Там restart станет False, но здесь-то цикл продолжится. Этого можно избежать, если сразу при нажатии на
    # крестик выходить из игры, но тогда процесс завершится ошибкой, что не очень хорошо
    if restart:
        game_loop()
    if restart:
        end_game_loop()
